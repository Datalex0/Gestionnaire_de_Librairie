...
df_books.drop(columns=['number', 'cites', 'volume', 'editor', 'isbn', 'series', 'publisher', 'type', 'title', 'pages', 'year', 'booktitle', 'url', 'authors'], axis=1, inplace=True)
...
if choix == 'Les Ouvrages':
    # choix_ouvrages = st.sidebar.radio(
    # "Quelle stat voulez-vous ?",
    # ("Le nombre total d'Ouvrages", "Le Nombre d'Ouvrages par Type"))
    
    #if choix_ouvrages == "Le nombre total d'Ouvrages":
    #elif choix_ouvrages == "Le Nombre d'Ouvrages par Type":
    ...
    with col1:
        # Avec Python :
        #nb_ouvrages_type = df_books['Type'].value_counts()
        #df_types = pd.DataFrame({'Type': nb_ouvrages_type.index, 'Count': nb_ouvrages_type.values})
